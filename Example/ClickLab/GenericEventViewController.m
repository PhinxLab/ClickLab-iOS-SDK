...
#import "GenericEventViewController.h"
#import "ParamCell.h"
#import "ClickLab.h"

@interface GenericEventViewController () <UITableViewDelegate, UITableViewDataSource>
...
@implementation GenericEventViewController
...
- (IBAction)trackEvent:(id)sender {
    // Hide keyboard
    [self hideKeyboard];
    // Validate custom event type field
    if ([self.eventTypeTextField.text isEqualToString:@""]) {
        [self showAlertWithMessage:@"Please complete event type field."];
    } else {
        [self trackEvent];
...
- (void)trackEvent {
    CLBGenericEvent *event = [CLBGenericEvent genericEventWithName:self.eventTypeTextField.text];
...
- (void)showSuccessAlert {
    ...
    // Create OK action
    __weak GenericEventViewController *weakSelf = self;
